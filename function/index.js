...
export const handler = async () => {
  ...
  */
  ...
  /*
    Reconcile users
  */
  const identityStoreUsers = await getIdentityStoreValuesByType("users").catch(
    (error) => {
      throw new Error(error);
    },
  );
  ...
  */
  const refreshedGroups = await getIdentityStoreValuesByType("groups").catch(
    (error) => {
      throw new Error(error);
    },
  );
  const refreshedUsers = await getIdentityStoreValuesByType("users").catch(
    (error) => {
      throw new Error(error);
    },
  );
  ...
  for await (const group of refreshedGroups) {
    ...
    if (!githubTeam) {
      ...
      const _syncMembership = await sync(
        "membership",
        reconcileMembership,
      ).catch((error) => {
        throw new Error(error);
